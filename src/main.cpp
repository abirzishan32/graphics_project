...
unsigned int createBillboardVAO();
...
void drawEscalatorAccessArea(Shader& shader, unsigned int cubeVAO, unsigned int quadVAO, unsigned int billboardVAO);
...
BasicCamera camera(LOT_WIDTH + 30.0f, 1.7f, LOT_DEPTH * 0.78f);
...
int main()
{
    ...
    escalatorBillboardVAO = createBillboardVAO();
    ...
    glDeleteVertexArrays(1, &escalatorBillboardVAO);
...
unsigned int createBillboardVAO()
{
    ...
    // UVs are explicitly [0,1] so fashion textures can be mapped directly later.
    float vertices[] = {
...
void drawEscalatorAccessArea(Shader& shader, unsigned int cubeVAO, unsigned int quadVAO, unsigned int billboardVAO)
{
    ...
    // Anchor the escalator to the mall right facade so top lands exactly at first floor.
    float firstFloorY = FLOOR_TO_FLOOR_HEIGHT;
    float mallRightWallX = LOT_WIDTH + WALL_THICKNESS * 0.5f;
    float connectZ = LOT_DEPTH * 0.78f;

    float topLandingX = mallRightWallX + 2.0f;
    float baseX = LOT_WIDTH + 26.0f;
    float escalatorRun = baseX - topLandingX;
    float escalatorRise = firstFloorY;
    float escalatorAngleDeg = glm::degrees(std::atan2(escalatorRise, escalatorRun));
    float escalatorLen = std::sqrt(escalatorRun * escalatorRun + escalatorRise * escalatorRise);
    float laneOffsetZ = 1.45f;
    float laneWidth = 2.4f;

    // 1) Vibrant green terrain plane for this outer section
    glm::mat4 terrainModel = glm::mat4(1.0f);
    terrainModel = glm::translate(terrainModel, glm::vec3(LOT_WIDTH + 20.0f, -0.06f, LOT_DEPTH * 0.84f));
    terrainModel = glm::scale(terrainModel, glm::vec3(54.0f, 1.0f, 48.0f));
    drawQuad(shader, quadVAO, terrainModel,
             glm::vec3(0.18f, 0.64f, 0.23f), 0, 0.25f, 0.75f, 0.08f, 6.0f);

    // 2) Main pedestrian pathway leading to escalator base
    float pathStartX = baseX + 11.0f;
    float pathLen = pathStartX - baseX;
    glm::mat4 pathModel = glm::mat4(1.0f);
    pathModel = glm::translate(pathModel, glm::vec3(baseX + pathLen * 0.5f, 0.015f, connectZ));
    ...
    // 3) Branching side-road from existing front road to this escalator approach
    float roadJunctionZ = LOT_DEPTH + 9.5f;
    float connectorX = baseX + 8.0f;
    float branchStartX = LOT_WIDTH * 0.62f;
    ...
    float branchZLen = connectZ - roadJunctionZ;
    ...
    // 4) Dual escalator lanes: left lane up, right lane down
    auto drawEscalatorLane = [&](float laneZ, bool upDirection) {
        glm::vec3 rampPos(topLandingX + escalatorRun * 0.5f, escalatorRise * 0.5f, laneZ);

        // Structural inclined ramp along +X, tilted in Y by Z-rotation.
        drawCubeRotated(shader, cubeVAO, rampPos,
                        glm::vec3(escalatorLen, 0.35f, laneWidth),
                        glm::vec3(0.0f, 0.0f, escalatorAngleDeg),
                        glm::vec3(0.36f, 0.37f, 0.40f), 3, 0.08f, 0.48f, 0.55f, 64.0f);

        // Side rails
        drawCubeRotated(shader, cubeVAO, rampPos + glm::vec3(0.0f, 0.45f, -laneWidth * 0.48f),
                        glm::vec3(escalatorLen, 0.9f, 0.06f),
                        glm::vec3(0.0f, 0.0f, escalatorAngleDeg),
        ...
        // 5) Moving steps with modulo loop (opposite directions per lane)
        const int stepCount = 22;
        float tNow = glfwGetTime();
        float motion = std::fmod(tNow * 0.45f, 1.0f);
        ...
        for (int i = 0; i < stepCount; ++i) {
            ...
            float stepY;
            if (upDirection) {
                stepX = baseX - p * escalatorRun;
                stepY = p * escalatorRise + 0.20f;
            } else {
                stepX = topLandingX + p * escalatorRun;
                stepY = (1.0f - p) * escalatorRise + 0.20f;
            }

            drawCubeRotated(shader, cubeVAO,
                            glm::vec3(stepX, stepY, laneZ),
                            glm::vec3(0.55f, 0.12f, laneWidth - 0.28f),
                            glm::vec3(0.0f, 0.0f, escalatorAngleDeg),
                            stepColor, 6, 0.1f, 0.56f, 0.8f, 120.0f);
        }
    };

    // Facing the mall from outside: left lane (lower Z) moves up, right lane (higher Z) moves down.
    drawEscalatorLane(connectZ - laneOffsetZ, true);
    drawEscalatorLane(connectZ + laneOffsetZ, false);

    // Top landing and connection pad that touches the mall first-floor side entry.
    drawCube(shader, cubeVAO,
             glm::vec3(topLandingX, firstFloorY + 0.08f, connectZ),
             glm::vec3(5.8f, 0.16f, 6.0f),
             glm::vec3(0.28f, 0.29f, 0.31f), 3, 0.1f, 0.5f, 0.35f, 40.0f);

    drawCube(shader, cubeVAO,
             glm::vec3(mallRightWallX + 0.35f, firstFloorY + 0.08f, connectZ),
             glm::vec3(0.7f, 0.16f, 5.6f),
             glm::vec3(0.30f, 0.31f, 0.33f), 3, 0.1f, 0.5f, 0.35f, 40.0f);

    // Visual first-floor side entry marker on wall
    drawCube(shader, cubeVAO,
             glm::vec3(mallRightWallX + 0.18f, firstFloorY + 1.35f, connectZ),
             glm::vec3(0.25f, 2.4f, 3.4f),
             glm::vec3(0.78f, 0.80f, 0.84f), 3, 0.12f, 0.55f, 0.45f, 48.0f);

    // 6) Billboard rows along both sides of pathway (3 per side), UV-ready mesh
    shader.setInt("useTexture", 0);
    ...
    float posterZs[2] = { connectZ - 4.6f, connectZ + 4.6f };
    for (int side = 0; side < 2; ++side) {
        for (int i = 0; i < 3; ++i) {
            float x = pathStartX - 2.6f - i * 3.2f;
...
void drawScene(Shader& shader, unsigned int cubeVAO, unsigned int quadVAO, unsigned int cylVAO, unsigned int wedgeVAO) {
    ...
    drawEscalatorAccessArea(shader, cubeVAO, quadVAO, escalatorBillboardVAO);
